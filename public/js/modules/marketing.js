...
Modules.Marketing = (function () {
  ...
  function _pointsConfigurationHtml() {
    var cfg = _pointsConfigData();
    var input = _marketingInputStyle() + 'height:40px;';
    var label = _marketingModalLabelStyle();
    var card = 'background:#fff;border:1px solid #EAE4DA;border-radius:16px;padding:16px;box-shadow:0 12px 30px rgba(31,31,31,.06);';
    ...
    function field(id, title, value, type, suffix) {
      return '<div><label style="' + label + '">' + _esc(title) + '</label><div style="display:flex;align-items:center;gap:8px;"><input id="' + id + '" type="' + (type || 'text') + '" value="' + _esc(value) + '"' + changeAttr() + ' style="' + input + '">' + (suffix ? '<span style="font-size:13px;font-weight:600;color:#6F6860;white-space:nowrap;">' + _esc(suffix) + '</span>' : '') + '</div></div>';
    }
    function select(id, title, html, extra) {
      return '<div><label style="' + label + '">' + _esc(title) + '</label><select id="' + id + '"' + (extra || changeAttr()) + ' style="' + input + '">' + html + '</select></div>';
    }
    function section(title, subtitle, inner) {
      return '<section style="' + card + '">' +
        '<div style="margin-bottom:12px;">' +
          '<div style="font-size:14px;font-weight:700;color:#1F1F1F;line-height:1.2;">' + _esc(title) + '</div>' +
          '<div style="font-size:12px;color:#6F6860;line-height:1.45;margin-top:4px;">' + _esc(subtitle) + '</div>' +
        '</div>' + inner +
      '</section>';
    }
    var previewCard =
    ...
    var redeem = section('Como o cliente usa os pontos', 'Configure a conversão dos pontos em desconto e os limites por pedido.',
      '<div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;">' +
        field('pp-redeem', 'Pontos para gerar €1 de desconto', cfg.redeemRate, 'number') +
        field('pp-min', 'Mínimo de pontos para resgate', cfg.minimumPointsToUse, 'number') +
        field('pp-maxpct', 'Limite máximo de desconto por pedido', cfg.maxDiscountPct, 'number', '%') +
      '</div>');
    var validity = section('Validade e aplicação', 'Defina se pontos expiram e como o desconto entra no pedido.',
      '<div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;align-items:end;">' +
        select('pp-expire', 'Validade dos pontos', '<option value="false"' + (!cfg.pointsExpire ? ' selected' : '') + '>Não expiram</option><option value="true"' + (cfg.pointsExpire ? ' selected' : '') + '>Expiram</option>', ' onchange="Modules.Marketing._pointsToggleExpirationField();Modules.Marketing._pointsMarkConfigDirty();Modules.Marketing._pointsRefreshConfigPreview()"') +
    ...
    return '<div style="display:grid;grid-template-columns:minmax(0,1fr);gap:16px;align-items:start;">' +
      '<section style="display:flex;flex-direction:column;gap:14px;">' +
        ...
        '<div style="position:sticky;bottom:0;z-index:3;background:linear-gradient(180deg,rgba(250,248,244,0),#FAF8F4 28%);padding-top:14px;">' +
          '<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;background:#fff;border:1px solid #EAE4DA;border-radius:16px;padding:12px 14px;box-shadow:0 12px 30px rgba(31,31,31,.08);">' +
            '<div id="pp-config-dirty" style="font-size:12px;color:#6F6860;">Revise as regras antes de salvar.</div>' +
            '<button id="pp-save-btn" onclick="Modules.Marketing._savePointsConfig()" style="height:38px;padding:0 14px;border:none;border-radius:10px;background:#B42318;color:#fff;font-size:13px;font-weight:500;cursor:pointer;font-family:inherit;box-shadow:0 4px 12px rgba(180,35,24,.18);">Salvar configuração</button>' +
          '</div>' +
        '</div>' +
      '</section>' +
    '</div>';
  ...
  function _openPointsCustomerModal(id) {
    ...
    function metric(label, value, tone) {
      ...
      return '<div style="background:#FAF8F4;border:1px solid #EAE4DA;border-radius:14px;padding:12px 14px;"><div style="font-size:11px;font-weight:600;color:#6F6860;text-transform:uppercase;letter-spacing:.04em;">' + _esc(label) + '</div><div style="font-size:22px;font-weight:700;color:' + c.color + ';line-height:1;margin-top:7px;">' + _esc(value) + '</div></div>';
    }
    var movementsHtml = movements.length ? movements.map(function (m) {
      ...
      return '<div style="display:grid;grid-template-columns:104px 1fr auto;gap:10px;align-items:center;padding:10px 0;border-top:1px solid #EAE4DA;">' +
        '<div style="font-size:12px;color:#6F6860;">' + (ts ? _esc(UI.fmtDate(new Date(ts))) : '—') + '</div>' +
        '<div><span style="display:inline-flex;align-items:center;min-height:24px;padding:0 9px;border-radius:999px;background:' + tone.bg + ';border:1px solid ' + tone.border + ';color:' + tone.color + ';font-size:12px;font-weight:600;">' + _esc(_pointsMovementLabel(type)) + '</span><div style="font-size:12px;color:#6F6860;margin-top:4px;">Pedido: ' + _esc(m.orderDisplay || m.orderLabel || m.orderId || '—') + '</div></div>' +
        '<div style="font-size:13px;font-weight:700;color:' + (type === 'used' ? '#B42318' : '#1F6F43') + ';">' + (type === 'used' ? '-' : '+') + points + '</div>' +
      '</div>';
    }).join('') : '<div style="font-size:13px;color:#6F6860;padding:12px 0;border-top:1px solid #EAE4DA;">Sem movimentos registrados para este cliente.</div>';
    var ordersHtml = orders.length ? orders.map(function (o) {
      ...
      return '<div style="display:grid;grid-template-columns:104px 1fr auto;gap:10px;align-items:center;padding:10px 0;border-top:1px solid #EAE4DA;">' +
        ...
        '<div style="font-size:13px;font-weight:700;color:#1F1F1F;">' + UI.fmt(_pointsOrderFinalValue(o)) + '</div>' +
      '</div>';
    }).join('') : '<div style="font-size:13px;color:#6F6860;padding:12px 0;border-top:1px solid #EAE4DA;">Sem pedidos relacionados encontrados.</div>';
    var body = '<div style="display:flex;flex-direction:column;gap:14px;">' +
      '<section style="' + _marketingModalSectionStyle() + '">' +
        '<div style="display:flex;align-items:center;gap:12px;">' +
          '<span style="width:48px;height:48px;border-radius:16px;background:#FAF8F4;border:1px solid #EAE4DA;color:#8A6F5A;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;flex:0 0 auto;">' + _esc(_pointsInitials(customer.name || customer.email || 'Cliente')) + '</span>' +
          '<div style="min-width:0;"><div style="font-size:18px;font-weight:700;color:#1F1F1F;line-height:1.2;">' + _esc(customer.name || 'Cliente') + '</div><div style="font-size:13px;color:#6F6860;margin-top:4px;">' + _esc(customer.phone || customer.whatsapp || 'Sem telefone') + ' · ' + _esc(customer.email || 'Sem e-mail') + '</div></div>' +
        '</div>' +
      '</section>' +
      ...
      '</section>' +
      '<section style="' + _marketingModalSectionStyle() + '">' + _marketingSectionTitle('Histórico de movimentos', 'Entradas, usos e expirações vinculadas ao cliente.') + movementsHtml + '</section>' +
      '<section style="' + _marketingModalSectionStyle() + '">' + _marketingSectionTitle('Pedidos relacionados', 'Pedidos encontrados pelo cliente, vínculo ou telefone.') + ordersHtml + '</section>' +
    '</div>';
    var footer = '<div style="display:flex;justify-content:flex-end;"><button onclick="if(window._pointsCustomerModal)window._pointsCustomerModal.close()" style="height:38px;padding:0 14px;border:1px solid #EAE4DA;border-radius:10px;background:#fff;color:#1F1F1F;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;">Fechar</button></div>';
  ...
  function _pointsCustomersTableHtml() {
    ...
    if (!rows.length) return '<section style="' + _marketingCardStyle() + 'text-align:center;"><div style="font-size:15px;font-weight:700;color:#1F1F1F;margin-bottom:4px;">Nenhum cliente encontrado</div><div style="font-size:13px;color:#6F6860;">Ajuste a busca ou os filtros para ver clientes do programa.</div></section>';
    return '<div style="background:#fff;border:1px solid #EAE4DA;border-radius:16px;overflow:hidden;box-shadow:0 12px 30px rgba(31,31,31,.06);"><div style="overflow:auto;"><table style="width:100%;border-collapse:separate;border-spacing:0;min-width:920px;"><thead><tr style="background:#fff;">' +
      ...
      '</tr></thead><tbody>' + rows.map(function (c) {
        ...
        return '<tr onmouseenter="this.style.background=\'#FBF8F2\'" onmouseleave="this.style.background=\'#fff\'" style="background:#fff;transition:background .15s ease;">' +
  ...
  function _pointsMovementsTableHtml() {
    ...
    if (!list.length) return '<section style="' + _marketingCardStyle() + 'text-align:center;"><div style="font-size:15px;font-weight:700;color:#1F1F1F;margin-bottom:4px;">Sem movimentos registrados</div><div style="font-size:13px;color:#6F6860;">Entradas, usos e expirações aparecerão aqui quando houver histórico.</div></section>';
    return '<div style="background:#fff;border:1px solid #EAE4DA;border-radius:16px;overflow:hidden;box-shadow:0 12px 30px rgba(31,31,31,.06);"><div style="overflow:auto;"><table style="width:100%;border-collapse:separate;border-spacing:0;min-width:820px;"><thead><tr>' +
  ...
  function _pointsClientsSectionHtml() {
    var input = _marketingInputStyle() + 'height:40px;';
    var hasFilters = !!(_pointsUi.query || _pointsUi.balance !== 'all' || _pointsUi.movement !== 'all');
    var summary = _pointsSummary(_customers);
    return '<div style="display:flex;flex-direction:column;gap:16px;">' +
      '<section style="' + _marketingCardStyle() + '">' +
        '<div style="display:grid;grid-template-columns:minmax(320px,1.6fr) minmax(180px,.8fr) minmax(190px,.8fr) auto;gap:10px 12px;align-items:end;">' +
          '<input type="search" value="' + _esc(_pointsUi.query || '') + '" oninput="Modules.Marketing._pointsSetSearch(this.value)" placeholder="Buscar cliente por nome, telefone ou e-mail..." style="' + input + '">' +
          '<select onchange="Modules.Marketing._pointsSetBalance(this.value)" style="' + input + '"><option value="all"' + (_pointsUi.balance === 'all' ? ' selected' : '') + '>Todos os clientes</option><option value="with_balance"' + (_pointsUi.balance === 'with_balance' ? ' selected' : '') + '>Com saldo</option><option value="eligible"' + (_pointsUi.balance === 'eligible' ? ' selected' : '') + '>Elegíveis para resgate</option></select>' +
          '<select onchange="Modules.Marketing._pointsSetMovement(this.value)" style="' + input + '"><option value="all"' + (_pointsUi.movement === 'all' ? ' selected' : '') + '>Últimos movimentos</option><option value="earned"' + (_pointsUi.movement === 'earned' ? ' selected' : '') + '>Pontos gerados</option><option value="used"' + (_pointsUi.movement === 'used' ? ' selected' : '') + '>Pontos usados</option></select>' +
          (hasFilters ? '<button onclick="Modules.Marketing._pointsClearFilters()" style="height:40px;padding:0 14px;border:1px solid #EAE4DA;border-radius:10px;background:#fff;color:#6F6860;font-size:13px;font-weight:500;cursor:pointer;font-family:inherit;">Limpar filtros</button>' : '') +
        '</div>' +
        '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:12px;">' +
          _marketingChip(summary.customers + ' clientes') +
          _marketingChip(summary.activeCustomers + ' com pontos') +
          _marketingChip(summary.ready + ' elegíveis') +
          _marketingChip(summary.movements + ' movimentos') +
        '</div>' +
      '</section>' +
      '<section style="display:flex;flex-direction:column;gap:10px;">' +
        '<div><div style="font-size:14px;font-weight:700;color:#1F1F1F;">Clientes</div><div style="font-size:13px;color:#6F6860;line-height:1.45;margin-top:2px;">Veja quem acumulou pontos e quem já pode resgatar desconto.</div></div>' +
  ...
  function _promoToolbarHtml() {
    var customHtml = _promoUi.period === 'custom'
      ? '<div style="grid-column:1 / -1;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;">' +
          '<label style="' + _marketingLabelStyle() + '">Data inicial<input type="date" value="' + _esc(_promoUi.periodStart || '') + '" onchange="Modules.Marketing._setPromoPeriodStart(this.value)" style="' + _marketingInputStyle() + 'height:40px;margin-top:5px;"></label>' +
          '<label style="' + _marketingLabelStyle() + '">Data final<input type="date" value="' + _esc(_promoUi.periodEnd || '') + '" onchange="Modules.Marketing._setPromoPeriodEnd(this.value)" style="' + _marketingInputStyle() + 'height:40px;margin-top:5px;"></label>' +
        '</div>'
      : '';
    var clearHtml = _promoHasActiveFilters()
      ? '<button type="button" onclick="Modules.Marketing._clearPromoFilters()" style="height:40px;padding:0 14px;border:1px solid #EAE4DA;border-radius:10px;background:#fff;color:#6F6860;font-size:13px;font-weight:500;cursor:pointer;font-family:inherit;box-shadow:0 1px 2px rgba(31,31,31,.03);">Limpar filtros</button>'
      : '';
    return '<div style="' + _marketingCardStyle() + '">' +
      '<div style="display:grid;grid-template-columns:minmax(320px,1.4fr) repeat(3,minmax(140px,1fr)) auto;gap:10px;align-items:end;">' +
      '<div><input id="prm-search" type="search" value="' + _esc(_promoUi.query || '') + '" oninput="Modules.Marketing._setPromoSearch(this.value)" placeholder="Buscar por promoção, produto, tipo ou tag..." autocomplete="off" style="' + _marketingInputStyle() + 'height:40px;"></div>' +
      '<select onchange="Modules.Marketing._setPromoStatus(this.value)" style="' + _marketingInputStyle() + 'height:40px;"><option value="all"' + (_promoUi.status === 'all' ? ' selected' : '') + '>Status: Todas</option><option value="active"' + (_promoUi.status === 'active' ? ' selected' : '') + '>Ativas</option><option value="scheduled"' + (_promoUi.status === 'scheduled' ? ' selected' : '') + '>Agendadas</option><option value="paused"' + (_promoUi.status === 'paused' ? ' selected' : '') + '>Pausadas</option><option value="finalized"' + (_promoUi.status === 'finalized' ? ' selected' : '') + '>Finalizadas</option><option value="expired"' + (_promoUi.status === 'expired' ? ' selected' : '') + '>Expiradas</option></select>' +
      '<select onchange="Modules.Marketing._setPromoTypeFilter(this.value)" style="' + _marketingInputStyle() + 'height:40px;"><option value="all"' + (_promoUi.type === 'all' ? ' selected' : '') + '>Tipo: Todos</option><option value="pct"' + (_promoUi.type === 'pct' ? ' selected' : '') + '>Desconto (%)</option><option value="eur"' + (_promoUi.type === 'eur' ? ' selected' : '') + '>Desconto (€)</option><option value="2x1"' + (_promoUi.type === '2x1' ? ' selected' : '') + '>Leve 2 pague 1</option><option value="add1"' + (_promoUi.type === 'add1' ? ' selected' : '') + '>Leve mais</option><option value="frete"' + (_promoUi.type === 'frete' ? ' selected' : '') + '>Frete grátis</option></select>' +
      '<select onchange="Modules.Marketing._setPromoPeriod(this.value)" style="' + _marketingInputStyle() + 'height:40px;"><option value="all"' + (_promoUi.period === 'all' ? ' selected' : '') + '>Período: Todos</option><option value="today"' + (_promoUi.period === 'today' ? ' selected' : '') + '>Hoje</option><option value="week"' + (_promoUi.period === 'week' ? ' selected' : '') + '>Esta semana</option><option value="month"' + (_promoUi.period === 'month' ? ' selected' : '') + '>Este mês</option><option value="scheduled"' + (_promoUi.period === 'scheduled' ? ' selected' : '') + '>Agendadas</option><option value="expired"' + (_promoUi.period === 'expired' ? ' selected' : '') + '>Expiradas</option><option value="custom"' + (_promoUi.period === 'custom' ? ' selected' : '') + '>Personalizado</option></select>' +
      clearHtml +
      customHtml +
      '</div>' +
    '</div>';
        ...
        '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;flex-wrap:wrap;">' +
          '<div style="min-width:0;flex:1;">' +
            ...
            '<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:6px;">' +
              '<span style="font-size:11px;font-weight:600;padding:4px 9px;border-radius:999px;background:' + (normalizedType === 'pct' ? '#F0FAF4' : normalizedType === 'eur' ? '#F2F7FF' : normalizedType === '2x1' ? '#FFF0EE' : normalizedType === 'fixed' ? '#FFF7ED' : '#FFF7ED') + ';color:' + (normalizedType === 'pct' ? '#1F6F43' : normalizedType === 'eur' ? '#2F5F93' : normalizedType === '2x1' ? '#B42318' : normalizedType === 'fixed' ? '#B45309' : '#B45309') + ';">' + _esc(typeLabel) + '</span>' +
  ...
  function _promoTableRowHTML(promo) {
          ...
          : normalizedType === '2x1'
            ...
            : normalizedType === 'add1'
              ...
              : normalizedType === 'frete'
                ...
                : '—';
    var typeBg = normalizedType === 'pct' ? '#F0FAF4' : normalizedType === 'eur' ? '#F2F7FF' : normalizedType === '2x1' ? '#FFF0EE' : normalizedType === 'fixed' ? '#FFF7ED' : '#FFF7ED';
    var typeColor = normalizedType === 'pct' ? '#1F6F43' : normalizedType === 'eur' ? '#2F5F93' : normalizedType === '2x1' ? '#B42318' : normalizedType === 'fixed' ? '#B45309' : '#B45309';
    ...
    var productCount = _promoProductsForPromo(promo).length;
    var activeLabel = status.key === 'active' || status.key === 'scheduled' ? 'Pausar' : 'Ativar';
    return '<tr onclick="Modules.Marketing._openPromoModal(\'' + promoIdArg + '\', \'view\')" onmouseenter="this.style.background=\'#FBF8F2\'" onmouseleave="this.style.background=\'#fff\'" style="cursor:pointer;background:#fff;border-bottom:1px solid #EAE4DA;transition:background .15s ease;">' +
      _marketingTd('<input type="checkbox" disabled style="width:16px;height:16px;accent-color:#B42318;">') +
  ...
  function _promoImpactByProductHtml(promo) {
    ...
    return '<div style="display:flex;flex-direction:column;gap:10px;">' + products.map(function (product) {
      ...
      return '<div style="background:#fff;border:1px solid #EAE4DA;border-radius:12px;padding:12px 14px;">' +
        '<div style="font-size:13px;font-weight:600;color:#1F1F1F;">' + _esc(product.name || 'Produto') + '</div>' +
        '<div style="font-size:12px;color:#6F6860;margin-top:4px;">' + UI.fmt(price) + ' → ' + UI.fmt(calc.final) + ' · Desconto: ' + UI.fmt(calc.discount) + '</div>' +
        '<div style="font-size:12px;color:#6F6860;margin-top:4px;">Custo: ' + (noCost ? '—' : UI.fmt(cost)) + '</div>' +
        '<div style="font-size:12px;color:#6F6860;margin-top:4px;">Lucro estimado antes: ' + (profitBefore == null ? '—' : UI.fmt(profitBefore)) + ' · depois: ' + (profitAfter == null ? '—' : UI.fmt(profitAfter)) + '</div>' +
        '<div style="font-size:12px;color:#6F6860;margin-top:4px;">Margem antes: ' + (marginBefore == null ? '—' : marginBefore.toFixed(1).replace('.', ',') + '%') + ' · depois: ' + (marginAfter == null ? '—' : marginAfter.toFixed(1).replace('.', ',') + '%') + '</div>' +
        (alertTxt ? '<div style="font-size:12px;color:#B45309;font-weight:600;margin-top:6px;">' + _esc(alertTxt) + '</div>' : '') +
      '</div>';
    }).join('') + '</div>';
  }
  ...
  function _promoViewModalHtml(promo) {
    ...
    var alertHtml = alerts.length ? '<div style="display:flex;flex-direction:column;gap:6px;">' + alerts.map(function (a) {
      return '<div style="padding:10px 12px;border-radius:10px;background:' + (a.level === 'danger' ? '#FFF0EE' : a.level === 'warning' ? '#FFF7ED' : '#FAF8F4') + ';border:1px solid #EAE4DA;color:' + a.color + ';font-size:12px;font-weight:600;">' + _esc(a.text) + '</div>';
    }).join('') + '</div>' : '<div style="font-size:13px;color:#8A7E7C;">Sem alertas relevantes.</div>';
    return '<div style="display:flex;flex-direction:column;gap:14px;">' +
      '<section style="' + _marketingModalSectionStyle() + '">' +
        '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap;">' +
          '<div><div style="font-size:20px;font-weight:600;line-height:1.15;color:#1F1F1F;">' + _esc(promo.name || 'Promoção') + '</div><div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px;"><span style="font-size:11px;font-weight:600;padding:4px 9px;border-radius:999px;background:' + _promoStatusTone(status.key).bg + ';color:' + _promoStatusTone(status.key).color + ';">' + _esc(status.label) + '</span></div></div>' +
          '<div style="text-align:right;"><div style="' + _marketingModalLabelStyle() + '">Valor</div><div style="font-size:24px;font-weight:700;color:#B42318;">' + _esc(valueLabel) + '</div></div>' +
        '</div>' +
        '<div style="margin-top:10px;font-size:12px;color:#6F6860;">' + _esc(typeLabel) + '</div>' +
        '<div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-top:14px;">' +
          '<div><div style="font-size:10px;font-weight:600;color:#7A746B;text-transform:uppercase;letter-spacing:.02em;">Início</div><div style="font-size:12px;font-weight:500;color:#1F1F1F;">' + (promo.startDate || promo.startsAt ? _esc(UI.fmtDate(new Date(promo.startDate || promo.startsAt))) : '—') + '</div></div>' +
          '<div><div style="font-size:10px;font-weight:600;color:#7A746B;text-transform:uppercase;letter-spacing:.02em;">Fim</div><div style="font-size:12px;font-weight:500;color:#1F1F1F;">' + (promo.endDate || promo.endsAt ? _esc(UI.fmtDate(new Date(promo.endDate || promo.endsAt))) : '—') + '</div></div>' +
          '<div><div style="font-size:10px;font-weight:600;color:#7A746B;text-transform:uppercase;letter-spacing:.02em;">Produtos</div><div style="font-size:12px;font-weight:500;color:#1F1F1F;">' + _promoProductsForPromo(promo).length + '</div></div>' +
        '</div>' +
      '</section>' +
      _promoSalesSummaryHtml(promo) +
      '<section style="' + _marketingModalSectionStyle() + '">' +
        '<div style="' + _marketingModalLabelStyle() + 'margin-bottom:8px;">Alertas</div>' +
        alertHtml +
      '</section>' +
      '<section style="' + _marketingModalSectionStyle() + '">' +
        '<div style="' + _marketingModalLabelStyle() + 'margin-bottom:8px;">Impacto por produto</div>' +
        _promoImpactByProductHtml(promo) +
      '</section>' +
      '<section style="' + _marketingModalSectionStyle() + '">' +
        '<div style="' + _marketingModalLabelStyle() + 'margin-bottom:8px;">Histórico</div>' +
  ...
  function _promoEditModalHtml(promo) {
    ...
    var sectionStyle = _marketingModalSectionStyle();
    var fieldStyle = _marketingInputStyle();
    ...
    var prodListHtml = _products.length === 0
      ...
      : _products.map(function (prod) {
          ...
          return '<label data-product-name="' + _esc(String(prod.name || '').toLowerCase()) + '" style="display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid #EAE4DA;border-radius:12px;background:#fff;cursor:pointer;">' +
            ...
            '<div style="min-width:0;flex:1;">' +
            '<div style="font-size:13px;font-weight:600;color:#1F1F1F;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + _esc(prod.name) + '</div>' +
            '<div style="font-size:11px;color:#6F6860;">Refere-se a este produto na promoção</div>' +
            '</div>' +
            '</label>';
        }).join('');
    ...
    return `
      <div style="display:flex;flex-direction:column;gap:14px;">
        <section style="${sectionStyle}">
          <div style="font-size:20px;font-weight:600;line-height:1.15;color:#1F1F1F;">${_esc(promo && promo.id ? 'Editar promoção' : 'Criar promoção para vender mais rápido')}</div>
          <div style="font-size:12px;color:#6F6860;margin-top:6px;">Ative uma oferta para aumentar suas vendas agora</div>
          ...
          <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;">
            ${PROMO_TYPES.map(function (t) {
              ...
              return `<button type="button" data-ptype="${t.key}" onclick="Modules.Marketing._selectPromoType('${t.key}');Modules.Marketing._refreshPromoPreview();" style="min-height:84px;padding:12px;border:1px solid ${active ? '#B42318' : '#EAE4DA'};border-radius:14px;background:${active ? '#FFF0EE' : '#fff'};color:${active ? '#B42318' : '#1F1F1F'};font-family:inherit;cursor:pointer;text-align:left;display:flex;flex-direction:column;gap:6px;justify-content:center;">
                ...
                <div style="font-size:11px;color:#6F6860;">${_esc(t.hint)}</div>
              </button>`;
            }).join('')}
          </div>
        </section>
        ...
        <section style="${sectionStyle}">
          <div style="${labelStyle}">Definir oferta</div>
          <div id="prm-offer-fields"></div>
          <div style="margin-top:10px;">
            <label style="${labelStyle}">${minLabel}</label>
            <input id="prm-min" type="number" step="0.01" value="${promo.minOrder || ''}" oninput="Modules.Marketing._refreshPromoPreview()" style="${fieldStyle}">
            <div style="font-size:11px;color:#6F6860;margin-top:4px;">${minHelp}</div>
          </div>
        </section>

        <section style="${sectionStyle}">
          <div style="${labelStyle}">Período da promoção</div>
        ...
        <section style="${sectionStyle}">
          <div style="${labelStyle}">Onde aplicar essa promoção?</div>
          <div style="display:flex;gap:14px;flex-wrap:wrap;margin-bottom:12px;">
            <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px;font-weight:600;"><input type="radio" name="prm-apply" value="all"${(promo.applyTo || !promo.productIds || !promo.productIds.length) !== 'selected' ? ' checked' : ''} onchange="Modules.Marketing._refreshPromoPreview()" style="accent-color:#B42318;"> Todos os produtos</label>
            <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px;font-weight:600;"><input type="radio" name="prm-apply" value="selected"${(promo.applyTo === 'selected' || (promo.productIds && promo.productIds.length)) ? ' checked' : ''} onchange="Modules.Marketing._refreshPromoPreview()" style="accent-color:#B42318;"> Selecionar produtos</label>
          </div>
          <div id="prm-products-panel" style="display:${(promo.applyTo === 'selected' || (promo.productIds && promo.productIds.length)) ? 'block' : 'none'};">
            ...
            <div id="prm-products-count" style="font-size:11px;font-weight:600;color:#6F6860;margin:0 0 10px;">0 produtos selecionados</div>
        ...
        <section style="${sectionStyle}">
          <div style="${labelStyle}">Regras adicionais</div>
          <textarea id="prm-rules" style="${fieldStyle}min-height:88px;resize:vertical;" placeholder="Ex: aplicar só à noite, não combinar com cupom, etc.">${_esc(promo.rulesText || promo.rules || '')}</textarea>
        </section>

        <section style="${sectionStyle}display:flex;align-items:center;justify-content:space-between;gap:12px;">
          <div>
            <div style="font-size:13px;font-weight:600;color:#1F1F1F;">Status da promoção</div>
            <div style="font-size:11px;color:#6F6860;">Ativa, pausada ou agendada</div>
          </div>
        ...
        <section style="${sectionStyle}">
          <div style="${labelStyle}">Impacto da promoção</div>
    ...
    content.innerHTML = '<div style="display:flex;flex-direction:column;gap:16px;">' +
      '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;">' +
        '<div style="min-width:0;flex:1;">' +
          ...
          '<p style="font-size:13px;color:#6F6860;line-height:1.45;margin:0 0 10px;max-width:760px;">Fidelize clientes com pontos por pedido e acompanhe resgates sem complicar a operação.</p>' +
          '<div style="display:flex;gap:8px;flex-wrap:wrap;">' +
            _marketingChip(summary.activeCustomers + ' clientes com pontos') +
            _marketingChip(summary.totalBalance + ' pontos em circulação') +
            _marketingChip(summary.ready + ' elegíveis') +
          '</div>' +
        '</div>' +
  ...
  function _paintPromos() {
    ...
    var paginationHtml = paging.total ? '<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;padding:16px 18px;">' +
      ...
      '</div>' +
    '</div>' : '';
    var summaryChips = '<div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap;align-items:center;">' +
      _marketingChip(_promos.length + ' promoções') +
      _marketingChip(filtered.length + ' exibidas') +
      _marketingChip(summary.active + ' ativas') +
      _marketingChip(summary.products + ' produtos em promoção') +
      _marketingChip('Página ' + paging.page + ' de ' + paging.totalPages) +
    '</div>';
    content.innerHTML = '<div class="bf-page" style="display:flex;flex-direction:column;gap:16px;">' +
      ...
      (filtered.length === 0 ? _promoEmptyStateHtml() :
        '<section style="display:flex;flex-direction:column;gap:10px;">' +
        '<div><div style="font-size:14px;font-weight:700;color:#1F1F1F;">Promoções cadastradas</div><div style="font-size:13px;color:#6F6860;line-height:1.45;margin-top:2px;">Gerencie campanhas, status, período e produtos vinculados.</div></div>' +
  ...
  function _promoItemImpactHtml(product, state) {
    ...
    if (state.type === '2x1') {
      return '<div style="background:#fff;border:1px solid #EAE4DA;border-radius:12px;padding:12px 14px;">' +
        ...
        '<div style="font-size:12px;color:#B42318;font-weight:600;margin-top:4px;">Cliente leva 2 e paga 1</div>' +
  ...
  function _renderPromoOfferFields() {
    ...
    if (type === 'pct') {
      host.innerHTML = '<div style="display:grid;grid-template-columns:1fr auto;gap:12px;align-items:end;">' +
        ...
        '</div><div style="height:40px;padding:0 12px;border-radius:10px;background:#FAF8F4;border:1px solid #EAE4DA;font-size:13px;font-weight:600;color:#6F6860;display:flex;align-items:center;">%</div></div>';
      return;
    }
    ...
    if (type === '2x1') {
      host.innerHTML = '<div style="background:#fff;border:1px solid #EAE4DA;border-radius:12px;padding:12px 14px;color:#1F1F1F;font-size:13px;line-height:1.5;">O cliente compra 2 unidades e paga 1.</div>' +
        '<div style="margin-top:10px;font-size:12px;color:#6F6860;">Exija produto aplicado para definir onde a oferta vale.</div>';
      return;
    }
    ...
    if (type === 'add1') {
      host.innerHTML = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">' +
        ...
        '</div></div>' +
        '<div style="margin-top:10px;background:#fff;border:1px solid #EAE4DA;border-radius:12px;padding:12px 14px;color:#1F1F1F;font-size:13px;line-height:1.5;">Exemplo: Leve 3, pague 2.</div>';
      return;
    }

    if (type === 'frete') {
      host.innerHTML = '<div style="display:grid;grid-template-columns:1fr auto;gap:12px;align-items:end;">' +
        '<div><label style="' + labelStyle + '">Valor mínimo para frete grátis</label>' +
        '<input id="prm-min" type="number" step="0.01" value="' + (base.minOrder || '') + '" placeholder="Ex: 20,00" oninput="Modules.Marketing._refreshPromoPreview()" style="' + fieldStyle + '">' +
        '</div><div style="height:40px;padding:0 12px;border-radius:10px;background:#FAF8F4;border:1px solid #EAE4DA;font-size:13px;font-weight:600;color:#6F6860;display:flex;align-items:center;">Frete</div></div>' +
        '<div style="margin-top:10px;background:#fff;border:1px solid #EAE4DA;border-radius:12px;padding:12px 14px;color:#1F1F1F;font-size:13px;line-height:1.5;">O frete fica grátis quando o pedido atingir este valor.</div>';
      return;
    }
  ...
  function _openPromoModal(id, mode) {
    ...
    var footer = editMode
      ? '<div style="display:flex;flex-direction:column;gap:6px;align-items:stretch;">' +
        ...
        '</div>'
      : '<div style="display:flex;gap:10px;flex-wrap:wrap;">' +
          '<button onclick="Modules.Marketing._openPromoModal(\'' + (id || '') + '\', \'edit\')" style="flex:1;min-width:140px;padding:13px;border-radius:12px;border:none;background:#B42318;color:#fff;font-size:14px;font-weight:600;cursor:pointer;font-family:inherit;box-shadow:0 8px 18px rgba(180,35,24,.18);">Editar</button>' +
          '<button onclick="Modules.Marketing._duplicatePromo(\'' + (id || '') + '\')" style="flex:1;min-width:120px;padding:13px;border-radius:12px;border:1px solid #E6DDD3;background:#fff;color:#1F1F1F;font-size:14px;font-weight:600;cursor:pointer;font-family:inherit;">Duplicar</button>' +
          '<button onclick="Modules.Marketing._togglePromoStatus(\'' + (id || '') + '\')" style="flex:1;min-width:120px;padding:13px;border-radius:12px;border:1px solid ' + (promo.active !== false ? '#FEDF89' : '#D9F2E3') + ';background:' + (promo.active !== false ? '#FFF7ED' : '#F0FAF4') + ';color:' + (promo.active !== false ? '#B45309' : '#1F6F43') + ';font-size:14px;font-weight:600;cursor:pointer;font-family:inherit;">' + (promo.active !== false ? 'Pausar' : 'Ativar') + '</button>' +
          '<button onclick="if(window._promoModal)window._promoModal.close()" style="flex:1;min-width:120px;padding:13px;border-radius:12px;border:1px solid #E6DDD3;background:#fff;color:#1F1F1F;font-size:14px;font-weight:600;cursor:pointer;font-family:inherit;">Fechar</button>' +
        '</div>';
    ...
    window._promoModal = UI.modal({ title: title, body: body, footer: footer, maxWidth: '1120px' });
    if (editMode) {
      _renderPromoOfferFields();
  ...
  function _couponToolbarHtml(total, filtered) {
    return '<div style="' + _marketingCardStyle() + '">' +
      '<div style="display:grid;grid-template-columns:minmax(320px,1.8fr) repeat(2,minmax(140px,1fr)) auto;gap:10px 12px;align-items:end;">' +
        '<div><input id="cup-search" type="search" value="' + _esc(_couponUi.query || '') + '" oninput="Modules.Marketing._setCouponSearch(this.value)" placeholder="Buscar por código, tipo ou valor..." autocomplete="off" style="' + _marketingInputStyle() + 'height:40px;"></div>' +
        '<select onchange="Modules.Marketing._setCouponStatus(this.value)" style="' + _marketingInputStyle() + 'height:40px;"><option value="all"' + (_couponUi.status === 'all' ? ' selected' : '') + '>Status: Todos</option><option value="active"' + (_couponUi.status === 'active' ? ' selected' : '') + '>Ativos</option><option value="expired"' + (_couponUi.status === 'expired' ? ' selected' : '') + '>Expirados</option><option value="used"' + (_couponUi.status === 'used' ? ' selected' : '') + '>Esgotados</option></select>' +
        '<select onchange="Modules.Marketing._setCouponType(this.value)" style="' + _marketingInputStyle() + 'height:40px;"><option value="all"' + (_couponUi.type === 'all' ? ' selected' : '') + '>Tipo: Todos</option><option value="pct"' + (_couponUi.type === 'pct' ? ' selected' : '') + '>Percentual</option><option value="eur"' + (_couponUi.type === 'eur' ? ' selected' : '') + '>Valor fixo</option></select>' +
        ((_couponUi.query || _couponUi.status !== 'all' || _couponUi.type !== 'all') ? '<button type="button" onclick="Modules.Marketing._clearCouponFilters()" style="height:40px;padding:0 14px;border:1px solid #EAE4DA;border-radius:10px;background:#fff;color:#6F6860;font-size:13px;font-weight:500;cursor:pointer;">Limpar filtros</button>' : '<span></span>') +
      '</div>' +
      '<div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap;align-items:center;">' +
        _marketingChip(total + ' cupons') +
        _marketingChip(filtered + ' exibidos') +
      '</div>' +
    '</div>';
  }
  ...
  function _paintCupons() {
    ...
    content.innerHTML = '<div class="bf-page" style="display:flex;flex-direction:column;gap:16px;">' +
      '<div class="bf-page-header" style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;">' +
        '<div style="min-width:0;flex:1 1 420px;">' +
          '<div style="font-size:11px;font-weight:500;color:#A39B90;letter-spacing:.03em;margin-bottom:5px;">Ações de Vendas</div>' +
          '<h2 style="font-size:28px;font-weight:600;line-height:1.15;margin:0 0 6px;color:#1F1F1F;">Cupons</h2>' +
          '<p style="font-size:15px;font-weight:400;color:#7A746B;line-height:1.55;max-width:760px;margin:0;">Crie códigos de desconto, acompanhe validade e controle usos.</p>' +
        '</div>' +
        '<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;justify-content:flex-end;">' +
          '<button onclick="Modules.Marketing._openCuponModal(null, \'edit\')" style="height:38px;padding:0 14px;border:none;border-radius:10px;background:#B42318;color:#fff;font-size:13px;font-weight:500;cursor:pointer;font-family:inherit;box-shadow:0 4px 12px rgba(180,35,24,.18);">' + _subActionLabel() + '</button>' +
        '</div>' +
      '</div>' +
      ...
      (filtered.length === 0 ? _couponEmptyStateHtml(_cupons.length > 0) :
        '<div style="background:#fff;border:1px solid #EAE4DA;border-radius:16px;overflow:hidden;box-shadow:0 12px 30px rgba(31,31,31,.06);">' +
          '<div style="overflow:auto;">' +
            '<table class="bf-table" style="width:100%;border-collapse:separate;border-spacing:0;">' +
              '<thead><tr style="background:#fff;border-bottom:1px solid #EAE4DA;">' +
                '<th style="width:44px;padding:12px 16px;border-bottom:1px solid #EAE4DA;background:#fff;"><input type="checkbox" disabled style="width:16px;height:16px;accent-color:#B42318;"></th>' +
                _marketingTh('Código') +
                _marketingTh('Tipo') +
                ...
                _marketingTh('Ações', 'right') +
              '</tr></thead>' +
              '<tbody id="coupons-list">' + filtered.map(function (c) { return _couponTableRowHTML(c); }).join('') + '</tbody>' +
            '</table>' +
          '</div>' +
        '</div>') +
      '</div>';
  ...
  function _couponViewModalHtml(coupon) {
    ...
    return '<div style="display:flex;flex-direction:column;gap:14px;">' +
      '<section style="' + _marketingModalSectionStyle() + '">' +
        '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap;">' +
          '<div><div style="font-size:20px;font-weight:600;line-height:1.15;color:#1F1F1F;font-family:\'Courier New\',monospace;letter-spacing:.06em;">' + _esc(coupon.code || 'CUPOM') + '</div><div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px;"><span style="font-size:11px;font-weight:600;padding:4px 9px;border-radius:999px;background:' + status.bg + ';color:' + status.color + ';">' + _esc(status.label) + '</span></div></div>' +
          '<div style="text-align:right;"><div style="' + _marketingModalLabelStyle() + '">Valor</div><div style="font-size:24px;font-weight:700;color:#B42318;">' + _esc(_couponValueLabel(coupon)) + '</div></div>' +
        '</div>' +
        '<div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-top:14px;">' +
          '<div><div style="font-size:10px;font-weight:600;color:#7A746B;text-transform:uppercase;letter-spacing:.02em;">Tipo</div><div style="font-size:12px;font-weight:500;color:#1F1F1F;">' + _esc(_couponTypeLabel(coupon.type)) + '</div></div>' +
          '<div><div style="font-size:10px;font-weight:600;color:#7A746B;text-transform:uppercase;letter-spacing:.02em;">Pedido mínimo</div><div style="font-size:12px;font-weight:500;color:#1F1F1F;">' + (coupon.minOrder ? UI.fmt(coupon.minOrder) : '—') + '</div></div>' +
          '<div><div style="font-size:10px;font-weight:600;color:#7A746B;text-transform:uppercase;letter-spacing:.02em;">Máx. usos</div><div style="font-size:12px;font-weight:500;color:#1F1F1F;">' + (coupon.maxUses || 'Ilimitado') + '</div></div>' +
          '<div><div style="font-size:10px;font-weight:600;color:#7A746B;text-transform:uppercase;letter-spacing:.02em;">Validade</div><div style="font-size:12px;font-weight:500;color:#1F1F1F;">' + (coupon.expiry ? _esc(UI.fmtDate(new Date(coupon.expiry))) : '—') + '</div></div>' +
        '</div>' +
      '</section>' +
      '<section style="' + _marketingModalSectionStyle() + '">' +
        '<div style="' + _marketingModalLabelStyle() + 'margin-bottom:8px;">Uso</div>' +
        '<div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;">' +
          '<div style="background:#fff;border:1px solid #EAE4DA;border-radius:12px;padding:12px 14px;"><div style="font-size:10px;font-weight:600;color:#7A746B;text-transform:uppercase;letter-spacing:.02em;">Usos registrados</div><div style="font-size:22px;font-weight:700;color:#1F1F1F;margin-top:4px;">' + (coupon.usesCount || 0) + '</div></div>' +
          '<div style="background:#fff;border:1px solid #EAE4DA;border-radius:12px;padding:12px 14px;"><div style="font-size:10px;font-weight:600;color:#7A746B;text-transform:uppercase;letter-spacing:.02em;">Disponibilidade</div><div style="font-size:22px;font-weight:700;color:' + status.color + ';margin-top:4px;">' + _esc(status.label) + '</div></div>' +
        '</div>' +
      '</section>' +
    '</div>';
  }
  ...
  function _couponEditModalHtml(coupon) {
    ...
    return '<div style="display:flex;flex-direction:column;gap:14px;">' +
      '<section style="' + _marketingModalSectionStyle() + '">' +
        '<div style="font-size:20px;font-weight:600;line-height:1.15;color:#1F1F1F;">' + _esc(coupon && coupon.id ? 'Editar cupom' : 'Novo cupom') + '</div>' +
        '<div style="font-size:12px;color:#6F6860;margin-top:6px;">Configure código, valor, validade e limites de uso.</div>' +
        '<div style="margin-top:12px;"><label style="' + labelStyle + '">Código do cupom *</label><input id="cup-code" type="text" value="' + _esc(coupon.code || '') + '" placeholder="BRASIL10" style="' + fieldStyle + 'text-transform:uppercase;font-family:\'Courier New\',monospace;letter-spacing:.06em;font-weight:700;" oninput="this.value=this.value.toUpperCase()"></div>' +
      '</section>' +
      '<section style="' + _marketingModalSectionStyle() + '">' +
        '<div style="' + labelStyle + '">Desconto</div>' +
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">' +
          '<div><label style="' + labelStyle + '">Tipo</label><select id="cup-type" style="' + fieldStyle + '"><option value="pct"' + (coupon.type === 'pct' ? ' selected' : '') + '>Percentual (%)</option><option value="eur"' + (coupon.type === 'eur' ? ' selected' : '') + '>Valor fixo (€)</option></select></div>' +
          '<div><label style="' + labelStyle + '">Valor</label><input id="cup-value" type="number" step="0.01" value="' + _esc(coupon.value || '') + '" style="' + fieldStyle + '"></div>' +
        '</div>' +
      '</section>' +
      '<section style="' + _marketingModalSectionStyle() + '">' +
        '<div style="' + labelStyle + '">Condições</div>' +
        '<div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;">' +
          '<div><label style="' + labelStyle + '">Pedido mínimo (€)</label><input id="cup-min" type="number" step="0.01" value="' + _esc(coupon.minOrder || '') + '" style="' + fieldStyle + '"></div>' +
      ...
      '<section style="' + _marketingModalSectionStyle() + '">' +
        '<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;">' +
          '<div><div style="font-size:13px;font-weight:600;color:#1F1F1F;">Usos registrados</div><div style="font-size:11px;color:#6F6860;">Mantido automaticamente pelo sistema.</div></div>' +
          '<span style="display:inline-flex;align-items:center;justify-content:center;min-width:42px;height:32px;padding:0 12px;border-radius:10px;background:#FAF8F4;border:1px solid #EAE4DA;color:#1F1F1F;font-size:13px;font-weight:600;">' + (coupon.usesCount || 0) + '</span>' +
        '</div>' +
      '</section>' +
    '</div>';
  }
  ...
  function _openCuponModal(id, mode) {
    ...
    var footer = editMode
      ? '<div style="display:flex;flex-direction:column;gap:6px;align-items:stretch;">' +
        ...
        '</div>'
      : '<div style="display:flex;gap:10px;flex-wrap:wrap;">' +
          '<button onclick="Modules.Marketing._openCuponModal(\'' + _esc(id || '') + '\', \'edit\')" style="flex:1;min-width:140px;padding:13px;border-radius:12px;border:none;background:#B42318;color:#fff;font-size:14px;font-weight:600;cursor:pointer;font-family:inherit;box-shadow:0 8px 18px rgba(180,35,24,.18);">Editar</button>' +
          '<button onclick="if(window._cupomModal)window._cupomModal.close()" style="flex:1;min-width:120px;padding:13px;border-radius:12px;border:1px solid #E6DDD3;background:#fff;color:#1F1F1F;font-size:14px;font-weight:600;cursor:pointer;font-family:inherit;">Fechar</button>' +
        '</div>';
    if (window._cupomModal) window._cupomModal.close();
    window._cupomModal = UI.modal({ title: editMode ? (id ? 'Editar Cupom' : 'Novo Cupom') : 'Resumo do Cupom', body: body, footer: footer, maxWidth: '1120px' });
  }
  ...
  function _clearCouponFilters() {
    _couponUi = { query: '', status: 'all', type: 'all' };
    _paintCupons();
  }
  ...
  function _openUpsellShell(opts) {
    ...
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(31,31,31,.45);z-index:7000;display:flex;align-items:center;justify-content:center;padding:16px;opacity:0;transition:opacity .22s ease;';
    var maxWidth = opts.maxWidth || '1120px';
    var title = opts.title || '';
    var body = opts.body || '';
    var footer = opts.footer || '';
    overlay.innerHTML = '<div style="background:#fff;width:100%;max-width:' + maxWidth + ';max-height:calc(100vh - 32px);display:flex;flex-direction:column;overflow:hidden;border-radius:16px;box-shadow:0 20px 60px rgba(31,31,31,.22);transform:scale(.98);transition:transform .22s ease;">' +
      '<div style="flex:0 0 auto;padding:20px 24px 16px;border-bottom:1px solid #EAE4DA;display:flex;align-items:flex-start;justify-content:space-between;gap:16px;background:#fff;">' +
        '<div style="min-width:0;">' +
          '<div style="font-size:20px;font-weight:600;line-height:1.2;color:#1F1F1F;">' + _esc(title) + '</div>' +
          '<div style="font-size:13px;color:#6F6860;margin-top:4px;line-height:1.4;">' + _esc(opts.subtitle || '') + '</div>' +
        '</div>' +
        '<button class="ui-upsell-close" style="background:#fff;border:1px solid #EAE4DA;border-radius:10px;width:34px;height:34px;font-size:18px;color:#6F6860;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;">×</button>' +
      '</div>' +
      '<div class="ui-upsell-scroll" style="flex:1 1 auto;overflow-y:auto;padding:20px 24px;background:#fff;">' + body + '</div>' +
  ...
  function _upsellPerformanceSectionHtml() {
    ...
    var bestUpsellSub = hasBestUpsell ? ((perf.best.conversoes || 0) + ' pedidos finalizados') : 'Continue exibindo sugestões para gerar histórico';
    ...
    function insightText() {
      if (perf.disparos < 10) return 'Ainda não há dados suficientes para recomendações inteligentes. Continue exibindo sugestões para gerar histórico.';
      ...
      if (perf.adicionados > 0 && perf.conversoes === 0) return 'Clientes adicionam sugestões ao carrinho, mas não finalizam. Avalie preço final, frete e clareza do benefício.';
      ...
      return 'Continue exibindo sugestões para gerar histórico e identificar padrões de aceitação.';
    }
    var html = '<div style="display:flex;flex-direction:column;gap:14px;">' +
      '<div style="display:flex;align-items:flex-end;justify-content:space-between;gap:12px;flex-wrap:wrap;">' +
        ...
        '</div>' +
        '<label style="' + labelStyle + '"><span style="display:block;margin-bottom:4px;">Período</span><select onchange="Modules.Marketing._setUpsellPerfPeriod(this.value)" style="min-width:180px;' + inputStyle + '"><option value="today"' + (range.key === 'today' ? ' selected' : '') + '>Hoje</option><option value="yesterday"' + (range.key === 'yesterday' ? ' selected' : '') + '>Ontem</option><option value="last7"' + (range.key === 'last7' ? ' selected' : '') + '>Últimos 7 dias</option><option value="last30"' + (range.key === 'last30' ? ' selected' : '') + '>Últimos 30 dias</option><option value="thismonth"' + (range.key === 'thismonth' ? ' selected' : '') + '>Este mês</option><option value="lastmonth"' + (range.key === 'lastmonth' ? ' selected' : '') + '>Mês passado</option><option value="custom"' + (range.key === 'custom' ? ' selected' : '') + '>Personalizado</option></select></label>' +
      '</div>' +
      (range.key === 'custom'
        ? '<div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-bottom:12px;">' +
      ...
      '</div>' +
      '<div style="' + _marketingCardStyle() + 'padding:16px 18px;">' +
        _marketingSectionTitle('Funil do upsell', 'Acompanhe a evolução das sugestões até o pedido finalizado.') +
        '<div style="position:relative;display:flex;align-items:flex-start;gap:10px;flex-wrap:nowrap;overflow:auto;padding:8px 2px 2px;">' +
          '<div style="position:absolute;left:7%;right:7%;top:25px;height:2px;background:linear-gradient(90deg,#8A6F5A 0%,#A18362 35%,#6C8777 70%,#B42318 100%);opacity:.28;border-radius:999px;"></div>' +
  ...
  function _upsellSubtabsHtml() {
    ...
    return '<div style="display:inline-flex;align-items:center;gap:6px;background:#FAF8F4;border-radius:999px;padding:4px;box-shadow:inset 0 0 0 1px #EAE4DA;width:max-content;max-width:100%;overflow:auto;">' +
      tab('desempenho', 'Desempenho', 'monitoring') +
      tab('sugestoes', 'Sugestões', 'format_list_bulleted') +
    '</div>';
  }
  ...
  function _upsellToolbarHtml() {
    ...
    var customHtml = _upsellUi.period === 'custom'
      ? '<div style="grid-column:1 / -1;display:grid;grid-template-columns:repeat(2,minmax(180px,1fr));gap:10px 12px;margin-top:2px;">' +
          '<input type="date" value="' + _esc(_upsellUi.periodStart || '') + '" onchange="Modules.Marketing._setUpsellPeriodStart(this.value)" title="Data inicial" style="' + inputStyle + 'height:40px;">' +
          '<input type="date" value="' + _esc(_upsellUi.periodEnd || '') + '" onchange="Modules.Marketing._setUpsellPeriodEnd(this.value)" title="Data final" style="' + inputStyle + 'height:40px;">' +
        '</div>'
      : '';
    ...
    return '<div style="' + _marketingCardStyle() + '">' +
      '<div style="display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap;align-items:flex-end;">' +
        '<div style="display:grid;grid-template-columns:minmax(320px,1.8fr) repeat(2,minmax(150px,1fr)) auto;gap:10px 12px;flex:1;">' +
          '<input id="ups-search" type="search" value="' + _esc(_upsellUi.query || '') + '" oninput="Modules.Marketing._setUpsellSearch(this.value)" placeholder="Buscar sugestões pelo nome, produto ou tag..." autocomplete="off" autocapitalize="off" spellcheck="false" style="' + inputStyle + 'height:40px;">' +
          '<select onchange="Modules.Marketing._setUpsellStatus(this.value)" style="' + inputStyle + 'height:40px;"><option value="all"' + (_upsellUi.status === 'all' ? ' selected' : '') + '>Status: Todas</option><option value="active"' + (_upsellUi.status === 'active' ? ' selected' : '') + '>Status: Ativas</option><option value="scheduled"' + (_upsellUi.status === 'scheduled' ? ' selected' : '') + '>Status: Agendadas</option><option value="paused"' + (_upsellUi.status === 'paused' ? ' selected' : '') + '>Status: Pausadas</option><option value="expired"' + (_upsellUi.status === 'expired' ? ' selected' : '') + '>Status: Expiradas</option></select>' +
          '<select onchange="Modules.Marketing._setUpsellPeriod(this.value)" style="' + inputStyle + 'height:40px;"><option value="all"' + (_upsellUi.period === 'all' ? ' selected' : '') + '>Período: Todos</option><option value="today"' + (_upsellUi.period === 'today' ? ' selected' : '') + '>Período: Hoje</option><option value="week"' + (_upsellUi.period === 'week' ? ' selected' : '') + '>Período: Esta semana</option><option value="month"' + (_upsellUi.period === 'month' ? ' selected' : '') + '>Período: Este mês</option><option value="scheduled"' + (_upsellUi.period === 'scheduled' ? ' selected' : '') + '>Período: Agendadas</option><option value="expired"' + (_upsellUi.period === 'expired' ? ' selected' : '') + '>Período: Expiradas</option><option value="custom"' + (_upsellUi.period === 'custom' ? ' selected' : '') + '>Período: Personalizado</option></select>' +
          (hasFilters ? '<button onclick="Modules.Marketing._clearUpsellFilters()" style="height:40px;padding:0 14px;border:1px solid #EAE4DA;border-radius:10px;background:#fff;color:#6F6860;font-size:13px;font-weight:500;cursor:pointer;font-family:inherit;">Limpar filtros</button>' : '') +
          customHtml +
        '</div>' +
      '</div>' +
      '<div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap;align-items:center;">' +
        _marketingChip((_upsells || []).length + ' sugestões') +
        _marketingChip((_upsellFilteredList() || []).length + ' visíveis') +
        _marketingChip((_upsellSummary(_upsells).active || 0) + ' ativas') +
      '</div>' +
    '</div>';
  }
  ...
  function _upsellTableRowHtml(raw) {
    ...
    return '<tr onmouseenter="this.style.background=\'#FBF8F2\'" onmouseleave="this.style.background=\'#fff\'" style="background:#fff;cursor:pointer;transition:background .15s ease;" onclick="Modules.Marketing._openUpsellModal(\'' + _esc(String(rule.id)) + '\', \'view\')">' +
      _marketingTd('<input type="checkbox" onclick="event.stopPropagation()" style="width:16px;height:16px;accent-color:#C4362A;">') +
      _marketingTd('<div style="min-width:220px;"><div style="font-size:14px;font-weight:600;color:#1F1F1F;line-height:1.3;">' + _esc(_upsellDisplayName(rule)) + '</div><div style="font-size:12px;color:#6F6860;line-height:1.4;margin-top:3px;">Gatilho: ' + _esc(trigger) + '</div><div style="font-size:12px;color:#6F6860;line-height:1.4;">Sugestão: ' + _esc(suggested) + '</div></div>') +
  ...
  function _upsellEmptyStateHtml() {
    ...
    return '<div style="' + _marketingCardStyle() + 'text-align:center;padding:32px 24px;">' +
      ...
      '<div style="font-size:18px;font-weight:600;color:#1F1F1F;margin-bottom:6px;">' + (hasAny ? 'Nenhum upsell encontrado' : 'Nenhuma sugestão criada ainda') + '</div>' +
      '<div style="font-size:13px;color:#6F6860;line-height:1.5;margin:0 auto 18px;max-width:430px;">' + (hasAny ? 'Ajuste busca, status ou período para ver outras regras.' : 'Crie sugestões com benefício para aumentar o valor médio dos pedidos.') + '</div>' +
      (hasAny ? '<button onclick="Modules.Marketing._clearUpsellFilters()" style="background:#fff;color:#1F1F1F;border:1px solid #EAE4DA;padding:10px 16px;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;">Limpar filtros</button>' : '<button onclick="Modules.Marketing._openUpsellModal(null, \'edit\')" style="background:#C4362A;color:#fff;border:none;padding:10px 16px;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;">Criar sugestão</button>') +
    '</div>';
  }
  ...
  function _upsellSuggestionsMetricsHtml(summary) {
    ...
    var metrics = [
      { label: 'Total de sugestões', value: (_upsells || []).length, icon: 'inventory_2', color: '#8A6F5A' },
  ...
  function _paintUpsell() {
    ...
    try {
      ...
      try {
        tableHtml = filtered.length === 0 ? _upsellEmptyStateHtml() : '<div style="background:#fff;border:1px solid #EAE4DA;border-radius:16px;overflow:hidden;box-shadow:0 12px 30px rgba(31,31,31,.06);">' +
          '<div style="overflow:auto;">' +
            '<table class="bf-table" style="width:100%;border-collapse:separate;border-spacing:0;min-width:980px;">' +
              '<thead><tr style="background:#fff;border-bottom:1px solid #EAE4DA;">' +
                _marketingTh('', 'left') +
      ...
      suggestionsHtml = '<div class="bf-page" style="display:flex;flex-direction:column;gap:16px;">' +
        '<div class="bf-page-header" style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;">' +
          '<div style="min-width:0;flex:1 1 420px;">' +
            '<div style="font-size:11px;font-weight:500;color:#A39B90;letter-spacing:.03em;margin-bottom:5px;">Upsell</div>' +
            '<h2 style="font-size:28px;font-weight:600;line-height:1.15;margin:0 0 6px;color:#1F1F1F;">Sugestões</h2>' +
            '<p style="font-size:15px;font-weight:400;color:#7A746B;line-height:1.55;max-width:760px;margin:0;">Gerencie regras, benefícios, status e prioridades de exibição.</p>' +
          '</div>' +
          '<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">' +
            '<button onclick="Modules.Marketing._openUpsellModal(null, \'edit\')" style="height:38px;padding:0 14px;border:none;border-radius:10px;background:#B42318;color:#fff;font-size:13px;font-weight:500;cursor:pointer;font-family:inherit;box-shadow:0 4px 12px rgba(180,35,24,.18);transition:transform .15s ease,box-shadow .15s ease,background .15s ease;"><span class="mi" style="font-size:16px;vertical-align:-3px;margin-right:6px;">add</span>Nova sugestão</button>' +
          '</div>' +
        '</div>' +
      ...
      content.innerHTML = '<div style="display:flex;flex-direction:column;gap:14px;">' +
        '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;">' +
          '<div style="min-width:0;">' +
            ...
            '<p style="font-size:13px;color:#6F6860;line-height:1.45;margin:0 0 10px;">Crie sugestões inteligentes para aumentar o valor médio dos pedidos.</p>' +
            '<div style="display:flex;gap:8px;flex-wrap:wrap;">' +
              _marketingChip(_upsells.length + ' regras') +
              _marketingChip(summary.active + ' ativas') +
              _marketingChip(filtered.length + ' visíveis') +
            '</div>' +
          '</div>' +
          _upsellSubtabsHtml() +
        '</div>' +
  ...
  function _openUpsellModal(id, mode) {
    ...
    var inputStyle = _marketingInputStyle();
    var labelStyle = _marketingModalLabelStyle();
    var sectionTitleStyle = 'font-size:14px;font-weight:600;color:#1F1F1F;margin-bottom:6px;';
    var sectionDescStyle = 'font-size:13px;color:#6F6860;line-height:1.5;margin:0 0 12px;';
    var body;
    if (editMode) {
      body = '<div style="display:flex;flex-direction:column;gap:14px;">' +
        '<section style="' + sectionStyle + '">' +
          '<div style="' + sectionTitleStyle + '">Dados principais</div>' +
          '<p style="' + sectionDescStyle + '">Nomeie a regra e escolha a lógica que o cliente vai ver.</p>' +
          '<div><label style="' + labelStyle + '">Nome da regra</label><input id="ups-name" type="text" value="' + _esc(rule.name) + '" placeholder="Ex: Aumentar pedido com bebida" style="' + inputStyle + '"></div>' +
        '</section>' +
        '<section style="' + sectionStyle + '">' +
          '<div style="' + sectionTitleStyle + '">Tipo de upsell</div>' +
          '<div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;">' + ['complemento','upgrade','combo_sugerido','carrinho','valor_minimo'].map(function (t) {
            ...
            return '<button type="button" data-upstype="' + info.key + '" onclick="Modules.Marketing._selectUpsellType(\'' + info.key + '\')" style="padding:12px;border:1px solid ' + (active ? '#C4362A' : '#EAE4DA') + ';border-radius:12px;background:' + (active ? '#FFF0EE' : '#fff') + ';text-align:left;font-family:inherit;cursor:pointer;">' +
              '<div style="display:flex;align-items:center;gap:8px;"><span style="display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:10px;background:' + (active ? '#C4362A' : '#FAF8F4') + ';color:' + (active ? '#fff' : '#6F6860') + ';font-size:12px;font-weight:700;">' + _esc(info.label.slice(0,2).toUpperCase()) + '</span><span style="font-size:13px;font-weight:600;color:' + (active ? '#C4362A' : '#1F1F1F') + ';">' + _esc(info.label) + '</span></div>' +
              '<div style="font-size:12px;color:#6F6860;margin-top:6px;line-height:1.4;">' + _esc(info.desc) + '</div>' +
            '</button>';
          }).join('') + '</div>' +
        '</section>' +
        '<section style="' + sectionStyle + '">' +
          '<div style="' + sectionTitleStyle + '">Benefício da sugestão</div>' +
          '<div id="ups-benefit-block">' + _upsellBenefitSectionHtml(rule) + '</div>' +
        '</section>' +
        '<section style="' + sectionStyle + '">' +
          '<div style="' + sectionTitleStyle + '">Configurações da regra</div>' +
          '<div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;">' +
            '<div><label style="' + labelStyle + '">Produto gatilho</label><select id="ups-trigger-product" style="' + inputStyle + '"><option value="">—</option>' + _products.map(function (p) { return '<option value="' + _esc(String(p.id)) + '"' + (rule.triggerProductIds.indexOf(String(p.id)) >= 0 ? ' selected' : '') + '>' + _esc(p.name || 'Produto') + '</option>'; }).join('') + '</select></div>' +
            '<div style="grid-column:1 / -1"><label style="' + labelStyle + '">Produtos sugeridos</label><input id="ups-product-search" type="search" placeholder="Buscar produto sugerido..." oninput="Modules.Marketing._setUpsellProductSearch(this.value)" style="' + inputStyle + 'margin-bottom:10px;"><div id="ups-product-count" style="font-size:12px;font-weight:600;color:#6F6860;margin:0 0 10px;">' + selectedIds.length + ' produtos selecionados</div><div id="ups-product-list" style="max-height:220px;overflow:auto;border:1px solid #EAE4DA;border-radius:12px;padding:10px;background:#fff;">' + _upsellProductOptions(selectedIds) + '</div></div>' +
            _upsellLocationChooserHtml(rule) +
            '<div><label style="' + labelStyle + '">Momento da exibição</label><select id="ups-moment" style="' + inputStyle + '"><option value="trigger"' + (String(rule.displayMoment || '').toLowerCase() !== 'whatsapp' ? ' selected' : '') + '>Ao acionar o gatilho</option><option value="whatsapp"' + (String(rule.displayMoment || '').toLowerCase() === 'whatsapp' ? ' selected' : '') + '>Ao clicar no WhatsApp</option></select></div>' +
            '<div><label style="' + labelStyle + '">Mensagem ao cliente</label><input id="ups-message" type="text" value="' + _esc(rule.message) + '" placeholder="También te puede gustar" style="' + inputStyle + '"></div>' +
            '<div><label style="' + labelStyle + '">Prioridade</label><input id="ups-priority" type="number" step="1" min="0" value="' + rule.priority + '" style="' + inputStyle + '"></div>' +
            '<div><label style="' + labelStyle + '">Limite de exibição</label><input id="ups-limit" type="number" step="1" min="0" value="' + rule.displayLimit + '" style="' + inputStyle + '"></div>' +
            '<div><label style="' + labelStyle + '">Data de início</label><input id="ups-start" type="date" value="' + (rule.startDate || '') + '" style="' + inputStyle + '"></div>' +
            '<div><label style="' + labelStyle + '">Data de fim</label><input id="ups-end" type="date" value="' + (rule.endDate || '') + '" style="' + inputStyle + '"></div>' +
          '</div>' +
        '</section>' +
        '<section style="' + sectionStyle + '">' +
          '<div style="' + sectionTitleStyle + '">Produtos selecionados para upsell</div>' +
      ...
      body = '<div style="display:flex;flex-direction:column;gap:14px;">' +
        '<section style="' + sectionStyle + '">' +
          '<div style="font-size:22px;font-weight:700;line-height:1.15;color:#1F1F1F;">' + _esc(rule.name) + '</div>' +
          ...
          '</div>' +
          '<div style="margin-top:10px;font-size:13px;color:#6F6860;">' + _esc(quality.text) + '</div>' +
        ...
        '</section>' +
        '<section style="' + sectionStyle + '">' +
          '<div style="' + sectionTitleStyle + '">Resumo de vendas</div>' +
          _upsellSalesSummaryHtml(rule) +
        '</section>' +
        '<section style="' + sectionStyle + '">' +
          '<div style="' + sectionTitleStyle + '">Impacto por produto</div>' +
  ...
  return {
    ...
    _openCuponModal: _openCuponModal, _saveCupon: _saveCupon, _deleteCupon: _deleteCupon, _setCouponSearch: _setCouponSearch, _setCouponStatus: _setCouponStatus, _setCouponType: _setCouponType, _clearCouponFilters: _clearCouponFilters,
    ...
    _setUpsellSearch: _setUpsellSearch, _setUpsellStatus: _setUpsellStatus, _setUpsellPeriod: _setUpsellPeriod, _setUpsellPeriodStart: _setUpsellPeriodStart, _setUpsellPeriodEnd: _setUpsellPeriodEnd, _setUpsellPerfPeriod: _setUpsellPerfPeriod, _setUpsellPerfStart: _setUpsellPerfStart, _setUpsellPerfEnd: _setUpsellPerfEnd, _setUpsellTab: _setUpsellTab, _clearUpsellFilters: _clearUpsellFilters,
    ...
    _pointsSetTab: _pointsSetTab, _pointsSetSearch: _pointsSetSearch, _pointsSetBalance: _pointsSetBalance, _pointsSetMovement: _pointsSetMovement, _pointsClearFilters: _pointsClearFilters, _openPointsCustomerModal: _openPointsCustomerModal,
