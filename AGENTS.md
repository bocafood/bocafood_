...
- Campos devem ocupar somente o espaço necessário para preenchimento: documentos, número, código postal, país, prazo, unidade, quantidade, status, tipo, categoria e outros selects curtos em colunas menores; nomes, endereço, e-mail, fornecedor quando for busca, observações e descrições em colunas maiores. Evitar fazer todo campo ocupar metade ou a largura inteira sem necessidade.
- Alinhamento de campos: campos de uma mesma linha devem partir do mesmo eixo, ter altura consistente e não parecer “soltos”. Quando houver dois campos curtos, eles devem ficar agrupados à esquerda com largura proporcional, deixando espaço livre à direita em vez de esticar artificialmente.
