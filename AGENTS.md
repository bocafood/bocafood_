...
- Embalagem não é etapa de produção. No cadastro de `Etapas de produção`, as opções de ingredientes não devem listar itens da classe `Embalagem`; embalagens continuam no bloco próprio `Embalagens da receita`, aplicadas ao rendimento global.
...
- Em produção, separar rendimento da etapa e consumo da ficha: etapa/base de produção tem rendimento próprio e ingredientes próprios; ficha técnica/produto produzido informa quanto daquela etapa/base é consumido para produzir o produto. Ao concluir ordem de base/etapa, baixar os ingredientes dessa etapa e criar `entrada_base_producao`. Ao concluir ordem de produto final, baixar as bases/etapas controladas e apenas ingredientes diretos da ficha, criando `entrada_producao`; não baixar de novo os ingredientes internos das bases/etapas e não baixar embalagem nesse momento, porque embalagem sai junto na venda. Na venda, a baixa normal deve sair do produto final vendido (`produto_produzido`/receita/ficha técnica ou `produto_pronto`) e das embalagens vinculadas à ficha; etapas/ingredientes entram na regularização em cadeia quando a venda sem saldo precisar reconstruir histórico. Regularização por saldo negativo deve ser calculada por `stockItemType` para produto pronto, produto produzido, base/etapa, insumo e embalagem.
- Regularização em cadeia não deve criar compra nem financeiro. Quando uma venda deixa `produto_produzido` sem saldo, a pendência deve guardar `regularizationChain` com os movimentos técnicos necessários para reconstruir produção anterior: entrada técnica da etapa/base, saída técnica da etapa/base para o produto e entrada/saída técnica dos ingredientes usados. Quando a falta for de `base_producao`, a cadeia deve reconstruir os ingredientes da base. No modo automático, esses movimentos são gravados junto com a entrada de regularização; no modo manual, a tela `Estoque → Regularizações` deve aplicar a cadeia quando a usuária clicar em `Regularizar entrada`.
...
- A unidade da quantidade consumida pela ficha deve herdar/travar a unidade de rendimento da etapa quando existir. Só permitir edição manual da unidade quando a etapa não tiver unidade definida, para compatibilidade com dados antigos.
- Estoque mínimo/máximo de etapa/base deve ser calculado automaticamente pela soma das fichas que usam a etapa: mínimo/máximo da ficha multiplicado pela quantidade da etapa usada por unidade. Não reintroduzir campos manuais de mínimo/máximo da etapa dentro da ficha técnica.
- Ingredientes e embalagens usados dentro das etapas podem receber mínimo/máximo sugerido automaticamente a partir das fichas que usam essas etapas: demanda da etapa dividida pelo rendimento da etapa e multiplicada pela quantidade do ingrediente. A sugestão só deve ser usada quando não houver mínimo/máximo manual no item; valores manuais continuam tendo prioridade.
