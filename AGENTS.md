...
- Pedido manual criado no Admin deve tentar gerar movimentação de saída/regularização no ato do salvamento, mesmo nascendo com status `Pendente`; mudanças posteriores de status não devem duplicar a baixa quando `stockMovementCreated` já existir.
...
- Quando um item do cardápio tem `Montagem interna`, ela substitui apenas a baixa direta do produto/ficha principal. Escolhas/combo com vínculo de estoque no mesmo item ainda devem ser baixadas e regularizadas.
...
- Em produção, separar rendimento da etapa e consumo da ficha: etapa/base de produção tem rendimento próprio e ingredientes próprios; ficha técnica do produto vendido informa apenas quanto daquela etapa/base é consumido por unidade vendida (`stageUsageQuantity`). A origem `Montagem interna` é exclusiva e baixa somente os itens internos configurados. Se a ficha usa etapa/base com estoque controlado, a venda baixa a etapa/base pronta, não o produto produzido e nem os ingredientes crus da etapa. Produto produzido só baixa direto quando não houver etapa/base controlada representando esse consumo. Regularização por saldo negativo deve ser calculada por `stockItemType` para produto pronto, produto produzido, base/etapa, insumo e embalagem.
