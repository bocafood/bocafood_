...
- No onboarding/checklist, a primeira Temporada recomendada deve começar por `Aumentar Ticket` com prioridade `Melhor sobra` (`build: margin`), porque no início ainda há pouco histórico para orientar volume. O wizard pode pré-preencher esse padrão apenas quando ainda não existir nenhuma temporada.
- A última etapa do onboarding/checklist deve ser `Escolher regra inicial de estoque`, apontando para `Estoque > Configurações`. A copy deve recomendar que, se a usuária quer começar a vender já, use `Permitir venda sem saldo` com regularização em `Criar pendência`; depois ela pode voltar e mudar para `Bloquear quando zerar` quando compras, produção e estoque estiverem mais confiáveis.
- Depois que todas as etapas do onboarding/checklist inicial estiverem concluídas, o checklist flutuante deve desaparecer completamente; não deve ser substituído por outro card flutuante de rotina.
- A IA de Temporadas deve ser usada como camada de leitura e linguagem, não como motor de cálculo. Score, progresso, risco, metas, prazos, status, resultado e quantidade de jogadas continuam vindo do motor determinístico do BocaFood.
- Em Temporadas, o campo técnico `build` deve ser tratado na interface como `Prioridade da temporada`. Ele ajusta pesos e orienta por onde começar a leitura, mas não bloqueia outras jogadas quando produto, canal, horário, margem, recompra ou consistência mostrarem oportunidade mais forte.
- O contexto enviado para IA deve ser compacto: objetivo, dificuldade, período, progresso, risco, poucos produtos fortes/fracos, canais, horários/dias, sinais validados, plano de execução e tarefas. Não enviar listas completas de pedidos, clientes, snapshots grandes ou objetos duplicados.
- A recomendação de IA deve ser cacheada por snapshot diário usando `aiContextHash`. Se o hash não mudou, reutilizar a recomendação. Se havia apenas fallback local e o endpoint remoto passou a existir, permitir regenerar com IA real.
...
- Canais de venda podem ter `Conta bancária padrão` (`contaPadraoId/defaultAccountId/bankAccountId`). Pedidos manuais devem herdar essa conta pelo canal selecionado, e `Detalhes do pedido` deve permitir corrigir a conta no próprio pedido (`conta_id/contaBancariaId/accountId/bankAccountId`).
...
- Entradas financeiras criadas a partir de pedidos comuns devem nascer/continuar como `previsto`, mesmo quando o pedido estiver marcado como pago, para conferência e baixa manual no Financeiro. Preservar aliases de data do pedido e pagamento no movimento (`orderDate/dataPedido`, `paymentDate/dataPagamentoPedido`) sem preencher baixa automática.
- Ao cancelar pedido, a movimentação financeira vinculada deve ser marcada como `cancelada/estornada`, com recebido e saldo zerados, para não permanecer como entrada ativa.
- Quando uma entrada financeira vinculada a pedido estiver ativa, o detalhe do pedido não deve permitir editar `Forma de pagamento`, `Conta bancária`, `Status do pagamento` ou `Valor pago`. Para corrigir pagamento já enviado ao Financeiro, a usuária deve estornar a entrada no Financeiro; o estorno deve atualizar o pedido para `paymentStatus = estornado`, zerar valores pagos e liberar nova edição.
...
- `Produto sob encomenda` é configuração do cadastro do produto, não do pedido. O produto deve salvar `madeToOrder`, `productMadeToOrder`, `sobEncomenda`, `productionLeadDays` e `productionLeadTimeDays`, validando o prazo contra `Operação → Prazos e capacidade`. Ao criar pedido manual, o pedido apenas herda o maior prazo dos produtos selecionados, exige data de entrega/retirada compatível e salva os aliases de histórico (`madeToOrder`, `productionLeadDays`, `productionDeadlineDate`, `productionDeadlineType`) sem permitir editar esse prazo no pedido.
...
- A Fase 4 adiciona configuração em `Estoque → Configurações` para o comportamento da falta de saldo: `Criar pendência` como padrão seguro, `Aplicar automaticamente` para gerar `entrada_regularizacao` junto com a baixa, ou `Desligado` para não gerar pendência/entrada. Essa configuração é salva em `config/estoque.regularizationMode`.
...
- A regularização em cadeia só deve criar `entrada_regularizacao` para a parte da base/ingrediente que realmente ficaria negativa. Se a base ou ingrediente já tiver saldo suficiente, registrar apenas a saída técnica necessária quando a regularização for aplicada. No modo `Criar pendência`, a cadeia não pode alterar o saldo em memória usado por outros itens do mesmo pedido; isso só acontece no modo `Aplicar automaticamente`.
- A ação `Ignorar` em `Estoque → Regularizações` deve apenas marcar o item como `ignorada` no pedido e na movimentação de saída relacionada. Ela não pode criar entrada de estoque, compra, financeiro, ajuste de inventário ou alterar saldo.
- A listagem `Estoque → Itens` não deve exibir IDs técnicos abaixo do nome do item. Mostrar o código sequencial `stockCode`/`codigoEstoque` gerado pelo sistema no formato `EST-0001`, persistido em `stock_settings` por `stockKey`.
...
- Essa leitura não substitui movimentações de estoque. Saídas de produto produzido, produto pronto, base de produção, insumo e embalagem continuam em estoque/produção; a venda do cardápio serve para responder qual produto/menu comercial vendeu mais, inclusive quando o mesmo item de estoque aparece em mais de um menu.
...
- A copy de receitas deve explicar rendimento, ingredientes, custo e produção de forma prática para a usuária, sem mencionar estruturas internas, coleções, Firestore, cálculo de bastidores ou integrações futuras. Quando o contexto for ficha técnica, receita ou base reaproveitável, usar a copy visível `Base de produção`/`Bases de produção`, não `Etapa de produção`; chaves internas antigas podem continuar como `etapas`/`components` para compatibilidade.
- `Produção → Bases de produção` é a área correta para cadastrar bases de produção reaproveitáveis, como massa, recheio, creme, molho e cobertura. A base salva rendimento, unidade, ingredientes e custo próprio diretamente no cadastro, mantendo compatibilidade com receitas existentes.
- A listagem de `Produção → Bases de produção` deve manter paginação e botões de ação no mesmo padrão de `Receitas de produção`: ações por ícone para editar, duplicar e excluir, sem ícone/imagem decorativa na coluna `Base`, com clique na linha abrindo o detalhe/cadastro. O modal de base deve seguir o padrão visual do modal `Compra — Editar registro de compra`: cards com cabeçalho, campos off-white, foco vermelho discreto e rodapé com `Cancelar` + ação principal.
- Embalagem não é base de produção. No cadastro de `Bases de produção`, as opções de ingredientes não devem listar itens da classe `Embalagem`; embalagens continuam no bloco próprio `Embalagens da receita`, aplicadas ao rendimento global.
- Receitas integram `Bases de produção`: ao selecionar uma base com ingredientes próprios, a ficha pode puxar esses ingredientes como sugestão inicial e preencher rendimento/unidade se os campos estiverem vazios. Nunca sobrescrever automaticamente ingredientes ou quantidades já preenchidos na receita, para evitar perda de edição manual e duplicidade de custo/consumo.
- Ordens de produção podem levar bases planejadas: novas ordens devem salvar `plannedStages` no snapshot e no topo da ordem, exibir `Bases planejadas` nos detalhes e registrar `componentName`/`productionStageName` nas saídas de ingredientes. Essa fase é rastreio/visibilidade; não alterar quantidades, custos ou baixas existentes sem fase explícita posterior.
...
- `Produção → Configurações` não deve voltar a exibir bases da receita; ali ficam categorias, unidades e cadastros auxiliares. Rotas antigas de etapas/componentes devem continuar redirecionando para `Produção → Bases de produção`.
...
- A previsão deve ser somente leitura até a usuária acionar uma ação controlada. Consultar previsão, alternar visão, filtrar, abrir cálculo ou simular quantidade não pode criar movimentação, baixar estoque, alterar pedido, publicar cardápio nem modificar Stripe.
...
- Baixa de estoque de combos deve priorizar vínculos internos salvos em `stockChoices`/`choiceStockRefs`, mas pode usar escolhas comerciais como fallback quando elas carregarem produto, receita, base de produção, insumo, embalagem ou produto pronto identificável.
...
- Quando uma escolha aparecer no pedido mas não tiver vínculo de estoque, ela não deve gerar baixa automática por suposição. O cadastro precisa deixar claro o item/receita/base de produção e a quantidade consumida.
...
- Pedido criado com status `Pendente` ou `Confirmado` não deve baixar estoque. A baixa de estoque do pedido só deve acontecer quando o status operacional chegar a `Em camino` ou `Listo para recoger`; pedidos antigos que já têm movimentações devem manter o histórico sem nova baixa automática.
- A baixa de estoque do pedido deve auditar movimentos esperados por item/componente e completar saídas faltantes mesmo quando o pedido já tem alguma `saida_venda`/`saida_base_venda`; nunca usar `stockMovementCreated` como motivo para pular a cadeia inteira.
- Quando um item do cardápio tem `Montagem interna`, ela substitui apenas a baixa direta do produto/ficha principal. Escolhas/combo com vínculo de estoque no mesmo item ainda devem ser baixadas e regularizadas, incluindo `produto_pronto`.
- Pedido manual deve salvar `orderDate`/`dataPedido` como data de registro do pedido, separada de `deliveryDate`/`pickupDate`; a entrada financeira do pedido usa essa data de registro, não a data de entrega ou retirada.
- Pedido manual não deve ser salvo sem canal de venda, forma de pagamento, conta bancária e status do pagamento selecionados; placeholders como `Sem forma definida` e `Sem conta definida` são estados inválidos.
- Pedido com status entregue/finalizado deve ficar bloqueado para edição de dados, itens, pagamento, origem e taxas. A única alteração permitida é mudar para `Cancelado`, acionando as rotinas de estorno de pagamento/financeiro e estoque.
- Em produção, separar rendimento da base e consumo da ficha: base de produção tem rendimento próprio e ingredientes próprios; ficha técnica/produto produzido informa quanto daquela base é consumido para produzir o produto. Ao concluir ordem de base, baixar os ingredientes dessa base e criar `entrada_base_producao`. Ao concluir ordem de produto final, baixar as bases controladas e apenas ingredientes diretos da ficha, criando `entrada_producao`; não baixar de novo os ingredientes internos das bases e não baixar embalagem nesse momento, porque embalagem sai junto na venda. Na venda, a baixa normal deve sair do produto final vendido (`produto_produzido`/receita/ficha técnica ou `produto_pronto`) e das embalagens vinculadas à ficha; bases/ingredientes entram na regularização em cadeia quando a venda sem saldo precisar reconstruir histórico. Regularização por saldo negativo deve ser calculada por `stockItemType` para produto pronto, produto produzido, base, insumo e embalagem.
- Regularização em cadeia não deve criar compra nem financeiro. Quando uma venda deixa `produto_produzido` sem saldo, a pendência deve guardar `regularizationChain` com os movimentos técnicos necessários para reconstruir produção anterior: entrada técnica da base, saída técnica da base para o produto e entrada/saída técnica dos ingredientes usados. Quando a falta for de `base_producao`, a cadeia deve reconstruir os ingredientes da base. No modo automático, esses movimentos são gravados junto com a entrada de regularização; no modo manual, a tela `Estoque → Regularizações` deve aplicar a cadeia quando a usuária clicar em `Regularizar entrada`.
- A loja pública deve respeitar `config/estoque.allowOutOfStockSales`/`sellWithoutStock`/`publicAllowOutOfStockSales`: quando falso ou ausente, produto calculável sem saldo fica bloqueado; quando verdadeiro, a venda sem saldo é permitida e a regularização segue o modo configurado. Produtos `madeToOrder`/`productMadeToOrder`/`sobEncomenda` nunca devem ser bloqueados por saldo público, porque dependem do prazo de produção.
- A unidade da quantidade consumida pela ficha deve herdar/travar a unidade de rendimento da base quando existir. Só permitir edição manual da unidade quando a base não tiver unidade definida, para compatibilidade com dados antigos.
- Estoque mínimo/máximo de base deve ser calculado automaticamente pela soma das fichas que usam a base: mínimo/máximo da ficha multiplicado pela quantidade da base usada por unidade. Não reintroduzir campos manuais de mínimo/máximo da base dentro da ficha técnica.
- Ingredientes e embalagens usados dentro das bases podem receber mínimo/máximo sugerido automaticamente a partir das fichas que usam essas bases: demanda da base dividida pelo rendimento da base e multiplicada pela quantidade do ingrediente. A sugestão só deve ser usada quando não houver mínimo/máximo manual no item; valores manuais continuam tendo prioridade.
